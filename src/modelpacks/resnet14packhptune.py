...
def preprocess(params):
    ...
    if params['mode'] == "train":
        ...
        train_ds = augment_and_performance(train_ds, batch_size, shuffle=True, 
            augment=True)
...
def train_model(params, train_ds, val_ds):
    ...
    tuner = kt.GridSearch(hypermodel=resnet_model,
                     objective=kt.Objective("val_f1_score", direction="max"),
                     directory= os.path.join(os.getcwd(), 'experiments', 
                        params['name'], params['mode']),
                     project_name='hp tuning')
...
def conv3x3(x, out_planes, stride=1, name=None):
    ...
    return layers.Conv2D(filters=out_planes, kernel_size=3, strides=stride, 
...
def basic_block(x, planes, stride=1, downsample=None, name=None):
    ...
    out = layers.BatchNormalization(momentum=0.9, epsilon=1e-5, 
        name=f'{name}.bn1')(out)
...
def make_layer(x, planes, blocks, stride=1, name=None):
    ...
    if stride != 1 or inplanes != planes:
        downsample = [
            layers.Conv2D(filters=planes, kernel_size=1, strides=stride, 
...
def resnet(x, blocks_per_layer, num_classes=1000, units=32, initial_kernel=7):
    x = layers.ZeroPadding2D(padding=3, name='conv1_pad')(x)
    x = layers.Conv2D(filters=units, kernel_size=initial_kernel, strides=2, 
    ...
    x = make_layer(x, units, blocks_per_layer[0], stride=1, name='layer1')
    x = make_layer(x, units*2, blocks_per_layer[1], stride=2, name='layer2')
    x = make_layer(x, units*4, blocks_per_layer[2], stride=2, name='layer3')
...
def model_architecture(params, lr, kernels, initial_kernel):
    ...
    outputs = resnet(inputs, [2, 2, 2], num_classes = 1, units=kernels, 
...
class ResnetHyperModel(HyperModel):
    ...
    def build(self, hp):
        ...
        #hp_batch = hp.Int("batch", 1, 8, step=2, sampling="log")
        hp_units = hp.Int("units", 8, 64, step=2, sampling="log")
        hp_initial_kernel = hp.Int("initial_kernel", 5, 8)

        model = model_architecture(self.params, lr=hp_learning_rate, 
            kernels=hp_units, initial_kernel=hp_initial_kernel)
